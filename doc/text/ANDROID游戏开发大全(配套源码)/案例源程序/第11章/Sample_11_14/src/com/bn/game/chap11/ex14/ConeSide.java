...
public class ConeSide {
    ...
    int textureId;//纹理id
	public ConeSide(float scale,float r, float h, int n, int textureId) {//大小，半径，高度，边数，纹理id
		...
		for(float angdeg=0;Math.ceil(angdeg)<360;angdeg+=angdegSpan)//侧面
		{
			...
			float [] norXYZ=VectorUtil.calConeNormal(//通过三个顶点求出法向量
					0, 0, 0,//底面圆心坐标
					vertices[count-6], vertices[count-5], vertices[count-4],//当前点坐标
					0, h, 0 //顶点坐标
					);
			for(int i=0;i<2;i++){//顶点和当前点的法向量
				normals[norCount++]=norXYZ[0];
				normals[norCount++]=norXYZ[1];
				normals[norCount++]=norXYZ[2];
			}
			norXYZ=VectorUtil.calConeNormal(//通过三个顶点求出法向量
					0, 0, 0,//底面圆心坐标
					vertices[count-3], vertices[count-2], vertices[count-1],//下一点坐标
					0, h, 0//顶点坐标
					);
			//下一点的法向量
			normals[norCount++]=norXYZ[0];
